...
const SiteReport = () => {
  ...
  const [chartData, setCharData] = useState([]); // State for chart data
//   const [medHis,setMedHis] = useState([]);
  ...
  function PatientData() {
    ...
    return axios.get(`${URL}/question/sitereport/${id}`).then((response) => response.data);
  }
...
function calculateChartData(patientData) {
  ...
  function printSiteHistory(){
    return chartData.map((answer,i)=>{
        return (<p key={i}>Based on your answers we believe that your {answer.name} is {((answer.students)%11)*10}%</p>)})
  }
